...
function install_build_tools {
	echo "Build Utilities... "
        sudo apt-get install build-essential libpcre3 libpcre3-dev libssl-dev libxslt1-dev libgd2-noxpm-dev libgeoip-dev -y    
}
...
function build_nginx_rtmp {
     ...
     sudo ./configure  --with-ld-opt=-Wl,-z,relro --prefix=/usr --conf-path=/etc/nginx/nginx.conf --http-log-path=/var/log/nginx/access.log --error-log-path=/var/log/nginx/error.log --lock-path=/var/lock/nginx.lock --pid-path=/run/nginx.pid --with-http_ssl_module --add-module=../nginx-rtmp-module
     sudo make -j 4
     sudo make install
}
install_build_tools
